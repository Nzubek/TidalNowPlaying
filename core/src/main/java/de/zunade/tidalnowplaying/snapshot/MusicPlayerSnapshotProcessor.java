...
public final class MusicPlayerSnapshotProcessor extends EntitySnapshotProcessor<Player> {

  public MusicPlayerSnapshotProcessor(EntitySnapshotRegistry registry) {
    super(registry);
  }
  ...
  public void process(Player player, float partialTicks, ExtrasWriter entityWriter) {
    this.registry().captureSnapshot(
        entityWriter,
        MusicSnapshotExtraKeys.MUSIC_PLAYER,
        player
    );
  }
}
